...
#define SIGBREAK    SIGHUP
  ...
  void HookupHandler() {
    if (already_hooked_up) {
      LOG_FATAL << "Tried to hookup signal handlers more than once.";
    }
    ...
    //  LOG_FATAL << "Cannot install SIGHUP handler.";
    //}
    //if (sigaction(SIGINT, &sa, NULL) == -1) {
    //  LOG_FATAL << "Cannot install SIGINT handler.";
    //}
    if (signal(SIGBREAK, handle_signal) == SIG_ERR) {
      LOG_FATAL << "Cannot install SIGBREAK handler.";
    }
  ...
  void UnhookHandler() {
    if (already_hooked_up) {
      ...
      //// Intercept SIGHUP and SIGINT
      //if (sigaction(SIGHUP, &sa, NULL) == -1) {
      //  LOG_FATAL << "Cannot uninstall SIGHUP handler.";
      //}
      //if (sigaction(SIGINT, &sa, NULL) == -1) {
      //  LOG_FATAL << "Cannot uninstall SIGINT handler.";
      //}
