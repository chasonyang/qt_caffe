...
void WindowDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  LOG_INFO << "Window data layer:" << std::endl
  ...
  if (!(infile >> hashtag >> image_index)) {
    LOG_FATAL << "Window file is empty";
  }
  do {
    CHECK_EQ(hashtag, "#");
    // read image path
    ...
    if (cache_images_) {
      ...
      if (!ReadFileToDatum(image_path, &datum)) {
        LOG_ERROR << "Could not open or find file " << image_path;
        return;
      }
  ...
  LOG_INFO << "Number of images: " << image_index+1;

  for (map<int, int>::iterator it = label_hist.begin();
      it != label_hist.end(); ++it) {
    LOG_INFO << "class " << it->first << " has " << label_hist[it->first]
              << " samples";
  }

  LOG_INFO << "Amount of context padding: "
  ...
  if (has_mean_file_) {
    ...
    LOG_INFO << "Loading mean file from: " << mean_file;
  ...
  for (int is_fg = 0; is_fg < 2; ++is_fg) {
    for (int dummy = 0; dummy < num_samples[is_fg]; ++dummy) {
      ...
      if (this->cache_images_) {
        ...
        if (!cv_img.data) {
          LOG_ERROR << "Could not open or find file " << image.first;
          return;
        }
      }
