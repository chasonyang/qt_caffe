...
void HDF5DataLayer<Dtype>::LoadHDF5FileData(const char* filename) {
  LOG_INFO << "Loading HDF5 file: " << filename;
  hid_t file_id = H5Fopen(filename, H5F_ACC_RDONLY, H5P_DEFAULT);
  if (file_id < 0) {
    LOG_FATAL << "Failed opening HDF5 file: " << filename;
  }
  ...
  if (this->layer_param_.hdf5_data_param().shuffle()) {
    ...
    LOG_INFO << "Successully loaded " << hdf_blobs_[0]->shape(0)
               << " rows (shuffled)";
  } else {
    LOG_INFO << "Successully loaded " << hdf_blobs_[0]->shape(0) << " rows";
  }
}
...
void HDF5DataLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  LOG_INFO << "Loading list of HDF5 filenames from: " << source;
  ...
  if (source_file.is_open()) {
    ...
    LOG_FATAL << "Failed to open source file: " << source;
  }
  source_file.close();
  num_files_ = hdf_filenames_.size();
  current_file_ = 0;
  LOG_INFO << "Number of HDF5 files: " << num_files_;
    ...
    if (current_row_ == hdf_blobs_[0]->shape(0)) {
      if (num_files_ > 1) {
        ...
        if (current_file_ == num_files_) {
          ...
          LOG_INFO << "Looping around to first file.";
        }
        LoadHDF5FileData(
