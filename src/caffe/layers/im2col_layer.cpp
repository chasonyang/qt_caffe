...
void Im2colLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  for (int n = 0; n < num_; ++n) {
    CHECK_EQ(bottom[0]->shape().size() - channel_axis_, num_spatial_axes_ + 1);
    CHECK_EQ(top[0]->shape().size() - channel_axis_, num_spatial_axes_ + 1);
    CHECK_EQ(kernel_shape_.count(), num_spatial_axes_);
    CHECK_EQ(pad_.count(), num_spatial_axes_);
    CHECK_EQ(stride_.count(), num_spatial_axes_);
