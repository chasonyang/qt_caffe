...
void DevicePair::compute(const vector<int> devices, vector<DevicePair>* pairs) {
      ...
      for (int j = i + 1; j < remaining.size(); ++j) {
        ...
        if (a.isMultiGpuBoard && b.isMultiGpuBoard) {
          if (a.multiGpuBoardGroupID == b.multiGpuBoardGroupID) {
            ...
            LOG_INFO << "GPU board: " << remaining[i] << ":" << remaining[j];
  ...
  LOG_INFO << "GPUs paired by boards, remaining: " << s.str();
  ...
  for (int d = 0; d < remaining_depth; ++d) {
    for (int i = 0; i < remaining.size(); ++i) {
      for (int j = i + 1; j < remaining.size(); ++j) {
        ...
        if (access) {
          pairs->push_back(DevicePair(remaining[i], remaining[j]));
          LOG_INFO << "P2P pair: " << remaining[i] << ":" << remaining[j];
          remaining.erase(remaining.begin() + j);
          break;
        }
      }
  ...
  LOG_INFO << "GPUs paired by P2P access, remaining: " << s.str();
  ...
  for (int d = 0; d < remaining_depth; ++d) {
    for (int i = 0; i < remaining.size(); ++i) {
      ...
      LOG_INFO << "Remaining pair: " << remaining[i] << ":"
                 << remaining[i + 1];
...
P2PSync<Dtype>::P2PSync(shared_ptr<Solver<Dtype> > root_solver,
  ...
  if (parent) {
    ...
    if (access) {
      ...
      LOG_INFO<< "GPU " << self << " does not have p2p access to GPU " << peer;
    }
