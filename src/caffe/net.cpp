...
void Net<Dtype>::Init(const NetParameter& in_param) {
  ...
  for (int layer_id = 0; layer_id < param.layer_size(); ++layer_id) {
    ...
    if (share_from_root) {
      LOG_INFO << "Sharing layer " << layer_param.name() << " from root net";
    ...
    if (share_from_root) {
      ...
      for (int top_id = 0; top_id < base_top.size(); ++top_id) {
        ...
        LOG_INFO << "Created top blob " << top_id << " (shape: "
...
int Net<Dtype>::AppendBottom(const NetParameter& param, const int layer_id,
  ...
  if (available_blobs->find(blob_name) == available_blobs->end()) {
    LOG_FATAL << "Unknown bottom blob '" << blob_name << "' (layer '"
...
Dtype Net<Dtype>::ForwardFromTo(int start, int end) {
  ...
  for (int i = start; i <= end; ++i) {
    // LOG_ERROR << "Forwarding " << layer_names_[i];
...
void Net<Dtype>::Backward() {
  ...
  if (debug_info_) {
    ...
    LOG_ERROR << "    [Backward] All net params (data, diff): "
...
//    LOG_INFO << "Copying source layer " << source_layer_name;
    ...
    for (int j = 0; j < target_blobs.size(); ++j) {
      if (!target_blobs[j]->ShapeEquals(source_layer.blobs(j))) {
        ...
        LOG_FATAL << "Cannot copy param " << j << " weights from layer '"
            << source_layer_name << "'; shape mismatch.  Source param shape is "
            << source_blob.shape_string() << "; target param shape is "
            << target_blobs[j]->shape_string() << ". "
    ...
    for (int j = 0; j < target_blobs.size(); ++j) {
      ...
      if (!H5Lexists(layer_hid, dataset_name.c_str(), H5P_DEFAULT)) {
        ...
        if (param_owners_[target_net_param_id] != -1) {
          ...
          LOG_FATAL << "Incompatible number of blobs for layer "
              << source_layer_name;
        }
      }
...
void Net<Dtype>::ToProto(NetParameter* param, bool write_diff) const {
  ...
  LOG_INFO << "Serializing " << layers_.size() << " layers";
  for (int i = 0; i < layers_.size(); ++i) {
...
const shared_ptr<Blob<Dtype> > Net<Dtype>::blob_by_name(
  ...
  if (has_blob(blob_name)) {
    ...
    LOG_WARNING << "Unknown blob name " << blob_name;
  }
  return blob_ptr;
}
...
const shared_ptr<Layer<Dtype> > Net<Dtype>::layer_by_name(
  ...
  if (has_layer(layer_name)) {
    ...
    LOG_WARNING << "Unknown layer name " << layer_name;
  }
  return layer_ptr;
}
